...
def sample_trajectory(policies, game, input_fn):
    ...
    while not state.is_terminal():
        ...
        if state.is_chance_node():
            ...
            target_probs.append(probs_full)
        else:
            inputs.append(input_fn(state))
            probs_sampling, probs_target = policies(inputs[-1])
            action = np.random.choice(game.num_distinct_actions(), p=probs_sampling)
...
def calculate_regrets(trajectory: Trajectory):
    sampling_probs = trajectory.sampling_probs
    target_probs = trajectory.target_probs
    actions = trajectory.taken_actions
...
class GameSampler:
    ...
    def run_traversals(self, iteration: int, policies):
        for _ in trange(self._num_traversals):
            trajectory = sample_trajectory(policies, self._game, self._input_fn)
